...
{
    // Obtener el siguiente id_edicion para esta formacion
    $result = $this->db
        ->query("SELECT COALESCE(MAX(id_edicion), 0) + 1 AS next_id FROM Edicion WHERE id_formacion = :id_formacion")
        ->bind(":id_formacion", $id_formacion)
        ->fetch();
    ...
    $stmt = $this->db->query("
        INSERT INTO Edicion (id_edicion, id_formacion, f_inicio, f_fin, horas)
        VALUES (:id_edicion, :id_formacion, :f_inicio, :f_fin, :horas)
    ");

    $stmt->bind(":id_edicion", $next_id);
...
public function getPersonasEdicion(int $id_formacion, int $id_edicion): array
{
    return $this->db
        ->query("
            ...
            WHERE pe.id_edicion = :id_edicion 
              AND pe.id_formacion = :id_formacion
        ")
        ->bind(":id_edicion", $id_edicion)
        ->bind(":id_formacion", $id_formacion)
        ->fetchAll();
